...
// --- New ESP32 Hardware-Safe Pin Assignments ---
const int TemperaturePin = 16;    // GPIO16 - Continuous rotation servo for temp rotary encoder
const int TimePin        = 17;    // GPIO17 - Continuous rotation servo for time rotary encoder
const int OnOffPin       = 18;    // GPIO18 - Position rotation servo for On/Off tactile switch
const int FanPin         = 19;    // GPIO19 - Position rotation servo for Fan tactile switch
...
// Text fields / parsing validation feedback
...
int lastTemperature = 230; // Startup default temperature
int lastFanSpeed = 3;      // Startup default fan speed
...
void setMaximumRoastingTime() {
  if (DebugLevel >= 1) {
    Serial.println("[Hardware] Turning Time Servo Clockwise to Max (20 mins)");
  }
  ServoTime.write(180); 
  delay(1500); 
  ServoTime.write(90); 
...
void adjustTemperature(int targetTemp) {
  ...
  long duration = abs(diff) * 100; // 100ms per degree Celsius
  
  if (DebugLevel >= 1) {
    Serial.printf("[Hardware] Temp change from %dC to %dC. Diff: %d. Running servo for %ld ms\n", lastTemperature, targetTemp, diff, duration);
  }
...
void adjustFanSpeed(int targetSpeed) {
  ...
  if (presses > 0) {
    if (DebugLevel >= 1) {
      Serial.printf("[Hardware] Adjusting Fan from %d to %d requiring %d presses\n", lastFanSpeed, targetSpeed, presses);
    }
...
// --- Profile Compilation & Parsing Utilities ---
void parseProfile(String input) {
  syntaxErrorMsg = "";
...
void executeStandaloneAPProcess() {
  ...
  Serial.printf("[Network Status] Operational Mode: STANDALONE AP\n");
  Serial.printf("[Network Status] Broadcasting Local SSID: %s\n", apSsid.c_str());
  Serial.printf("[Network Status] AP IP Address: %s\n", WiFi.softAPIP().toString().c_str());
}

bool executeWifiConnectionProcess() {
  WiFi.mode(WIFI_STA);
  Serial.printf("[Network Status] Operational Mode: WIFI STATION\n");
  Serial.printf("[Network Status] Connecting to target SSID: %s\n", clientSsid.c_str());
  ...
  if (WiFi.status() != WL_CONNECTED) {
    Serial.println("[Network Error] Connection failed or timed out after 60s.");
  ...
  Serial.printf("[Network Status] Successfully connected to network!\n");
  Serial.printf("[Network Status] Assigned Station IP Address: %s\n", WiFi.localIP().toString().c_str());
  return true;
}
...
String generateWifiSetupHtml() {
  ...
  html += "<title>Coffee Bean Roaster Configuration</title>";
...
String generateHtml() {
  String html = "<!DOCTYPE html><html><head><meta name='viewport' content='width=device-width, initial-scale=1.0'>";
  html += "<title>Coffee Bean Roaster</title>";
  html += "<style>";
  ...
  html += "textarea, button { padding: 10px; font-size: 16px; margin: 10px; }";
  ...
  html += ".card { background: white; padding: 20px; max-width: 420px; margin: auto; border-radius: 8px; box-shadow: 0 4px 8px rgba(0,0,0,0.1); border-top: 5px solid #5d4037; }";
  html += ".error { color: #d32f2f; font-weight: bold; margin: 10px 0; }";
  
  // FIXED: Added CSS styles for a smooth, high-performance browser-side loading animation
  ...
  html += "<h2>Coffee Bean Roaster</h2>";
  ...
  html += "<p style='margin-top: 5px;'><strong>Total Time mm:ss:</strong> " + getFormattedTime(totalRemainingTimeSec);
  
  // FIXED: If actively roasting or cooling down, cleanly render the CSS animated spinner element instead of plaintext characters
  ...
  if (currentState == MENU_SELECTION) {
    ...
    if (!isConfirmed) {
      ...
      for (int i = 0; i < instructionCount; i++) {
        html += "<li>mm:ss " + getFormattedTime(instructions[i].timeInSeconds) + " | " + String(instructions[i].temperature) + "C | Fan: " + String(instructions[i].fanSpeed) + "</li>";
      }
      html += "</ol>";
    ...
    html += "<h3>Active Profile: " + beanName + "</h3>";
    ...
    for (int i = 0; i < instructionCount; i++) {
      ...
      if (i == currentInstructionIdx && currentState == Roasting) {
        ...
        timeTrack = " -> <strong>Rem mm:ss: " + getFormattedTime(currentRemainingTimeSec) + "</strong>";
      }
      html += "<p>" + marker + "Step " + String(i+1) + ": mm:ss " + getFormattedTime(instructions[i].timeInSeconds) + " @ " + String(instructions[i].temperature) + "C, Fan: " + String(instructions[i].fanSpeed) + timeTrack + "</p>";
    }
    html += "</div>";
...
bool isAuthorizedClient(AsyncWebServerRequest *request) {
  ...
  if (!clientLocked) {
    ...
    clientLocked = true;
    Serial.printf("[Security Guard] First client connected! Locking session to IP: %s\n", allowedClientIP.toString().c_str());
    return true;
  }
  bool allowed = (clientIP == allowedClientIP);
  if (!allowed) {
    Serial.printf("[Security Guard] Blocked request from alternate IP: %s (Locked to: %s)\n", clientIP.toString().c_str(), allowedClientIP.toString().c_str());
  }
  return allowed;
}
...
void setup() {
  ...
  if (DebugLevel >= 1) {
    ...
    Serial.println("[System Configuration] Execution Mode: PRODUCTION RUN (Real-time Clock 1sec = 1000ms)");
  }

  // Allow allocation of all ESP32 hardware PWM timers
  ESP32PWM::allocateTimer(0);
  ESP32PWM::allocateTimer(1);
  ESP32PWM::allocateTimer(2);
  ESP32PWM::allocateTimer(3);

  // Servo Mount Attachment Declarations with standard RC pulse width ranges (500us - 2400us)
  ...
  // Read environment specifications out of memory
  EEPROM.begin(EEPROM_SIZE); 
  loadWifiFromEEPROM();
  ...
  // --- Web Request Endpoints Layout Mapping ---
  ...
  server.on("/reset", HTTP_GET, [](AsyncWebServerRequest *request){
    if (!isAuthorizedClient(request)) return;
    triggerReset = true;
    request->send(200, "text/plain", "System Resetting...");
  });
  ...
  server.on("/erase_all", HTTP_GET, [](AsyncWebServerRequest *request){
    ...
    request->send(200, "text/plain", "Settings Erased. Device Rebooting...");
  });
...
void loop() {
  ...
  if (triggerPause) {
    ...
    if (currentState == Roasting || currentState == Cooldown) {
      isPaused = !isPaused;
      if (DebugLevel >= 1) {
        Serial.println(isPaused ? "[State] Program Execution Paused." : "[State] Program Execution Resumed.");
      }
    }
  }
  ...
  if (triggerRun) {
    ...
    if (currentState == MENU_SELECTION && instructionCount > 0) {
      ...
      if (DebugLevel >= 1) {
        Serial.println("[Sequence] Starting Roasting Cycle. Pressing On/Off switch once.");
      }
  ...
  // --- Core State Machine Automation ---
  unsigned long currentMillis = millis();

  if (!isPaused && (currentState == Roasting || currentState == Cooldown)) {
    // FIXED: Cleaned out redundant array-indexing clock ticks since animation is handled in CSS
    ...
    if (currentState == Roasting) {
      ...
      if (elapsed >= stepDuration) {
        ...
        if (currentInstructionIdx < instructionCount) {
          ...
          if (DebugLevel >= 1) {
            Serial.println("[Sequence] All instruction phases finished. Triggering Cool Down.");
          }
          executeServoPress(ServoOnOff); 
      ...
      if (elapsed >= stepDuration) {
        ...
        totalRemainingTimeSec = 0;
        if (DebugLevel >= 1) {
          Serial.println("[Sequence] Cooldown sequence finished. Returning machine control back to user.");
        }
      }
    }
  }
  ...
  if (DebugLevel >= 1) {
    ...
    if (stateChanged || stepChanged) {
      ...
      Serial.printf(" | Dynamic Time Remaining: mm:ss %s\n", getFormattedTime(totalRemainingTimeSec).c_str());
