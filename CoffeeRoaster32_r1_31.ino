...
const String RELEASE_VERSION = "1.31"; // INCREMENTED: Updated to reflect Hardware Interrupt conversion
...
const int FanPin         = 19; 
const int KillSwitchPin  = 4;  // Assigned GPIO pin for physical emergency kill switch
const int FanPressDegree = 45;
const int OnOffPressDegree = 50;
...
// --- SSD1306 I2C OLED Configuration ---
...
// --- Kill Switch Interruption & Debounce Handling ---
bool killPromptActive = false;
volatile bool triggerKillSwitch = false; 
volatile unsigned long lastKillInterruptTime = 0;
const unsigned long debounceDelay = 250; // Stable hardware debounce threshold in milliseconds
...
void setMiniimumRoastingTime();
void parseProfile(String input);
void saveProfileToEEPROM();
void updateOledDisplay();
void triggerImmediateCooldown();
void IRAM_ATTR handleKillSwitchInterrupt();
...
void adjustTemperature(int targetTemp) {
  ...
  duration = abs(diff) * baseRatio;

  if (diff != 0) {
    if (diff > 0) {
      ServoTemperature.write(135);
    } else {
      ServoTemperature.write(45);
    }
    delay(duration);
...
// --- NEW: Hardware Interrupt Service Routine (ISR) ---
void IRAM_ATTR handleKillSwitchInterrupt() {
  unsigned long interruptTime = millis();
  // Filter out noisy contact bounce flags using lock intervals
  if (interruptTime - lastKillInterruptTime > debounceDelay) {
    triggerKillSwitch = true;
    lastKillInterruptTime = interruptTime;
  }
}

// Core emergency bypass module forcing structural environment configuration into Cooldown
void triggerImmediateCooldown() {
  if (DebugLevel >= 1) {
    Serial.printf("[%s] [EMERGENCY] Kill Switch confirmed. Bypassing remaining steps, driving machine directly to Cooldown.\n", getFormattedTime(totalRemainingTimeSec).c_str());
  }
  
  killPromptActive = false;
  isPaused = false; // Override pause if active during emergency strike
  
  // Directly actuate the physical power configuration
  executeServoPress(ServoOnOff, OnOffPressDegree); 
  adjustTemperature(150); 
  setMinimumRoastingTime(); 
  
  currentState = Cooldown;
  
  // Format current parameters for backup safety writeout
...
void setup() {
  ...
  // Setup pin type utilizing internal hardware pullups
  pinMode(KillSwitchPin, INPUT_PULLUP);
  // NEW: Map high-priority Falling Edge Hardware Interrupt directly to the ISR
  attachInterrupt(digitalPinToInterrupt(KillSwitchPin), handleKillSwitchInterrupt, FALLING);
...
void loop() {
  ...
  // --- NEW: Asynchronous Safe Kill Switch Execution Processing Block ---
  if (triggerKillSwitch) {
    triggerKillSwitch = false; // Reset atomic indicator flag immediately
    
    if (currentState == Roasting) {
      if (!killPromptActive) {
        killPromptActive = true;
        if (DebugLevel >= 1) {
          Serial.println("[EMERGENCY INTERRUPT] First strike registered. Awaiting confirmation input.");
        }
        updateOledDisplay(); 
      } else {
  ...
  // Clear prompt active logic flags if state drops naturally outside roasting loops
  if (currentState != Roasting) {
    killPromptActive = false;
  }
