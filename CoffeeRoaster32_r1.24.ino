...
const String RELEASE_VERSION = "1.24"; // INCREMENTED: Release version bump
...
void adjustTemperature(int targetTemp) {
  ...
  if (diff > 0) {
    ServoTemperature.write(135); // half speed 180 - 90/2.
  } else {
    ServoTemperature.write(45);  // half speed 90/2.
  }
  delay(duration);
...
//  Serial.printf("[%s] [Hardware] Adjust Temp to %dC from lastTemp %dC. Diff: %d. Running servo for %ld ms\n", getFormattedTime(totalRemainingTimeSec).c_str(), targetTemp, lastTemperature,  diff, duration);
