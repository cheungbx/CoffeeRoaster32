...
const String RELEASE_VERSION = "1.36"; // INCREMENTED: Release version bump
...
void loop() {
  ...
  if (triggerRun) {
    ...
    if (currentState == MENU_SELECTION && instructionCount > 0) {
      eraseRoastLogFile(); // Erase temporary logging file when Roasting is confirmed to start (*3 requirement)
  ...
  // Log values every 10 seconds during active Roasting state (*1 requirement)
